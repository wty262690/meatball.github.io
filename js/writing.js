...
function openallstory(){
    for(var i=0, length=StoryListName[0].value.length; i<length; i++){
        ...
        StoryListName[0].load.push(false);         
            ...
            xhttp.open("GET", "word/"+StoryListName[0].value[i]+".txt?ver="+config.version);
            xhttp.send(null);
    }
}
...
window.onload= function(){
    ...
    $('head').append('<link href="css/writing.css?v=' + config.version + '" rel="stylesheet" />');
